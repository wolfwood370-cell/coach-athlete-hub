...
export function RiskTable() {
  ...
  const actionNeeded = athletes.filter(
    (a) => a.acwrZone ==="high-risk"|| a.compliance30d < 60 || a.status ==="injured"  ).length;
  const highRiskPct =
    totalActive > 0
      ? Math.round(
          (athletes.filter(
            (a) => a.acwrZone ==="high-risk"|| a.acwrZone ==="detraining"          ).length /
            totalActive) *
            100
        )
      : 0;
        ...
        <CardHeader className="pb-3 pt-4 px-4 flex-row items-center justify-between space-y-0">
          ...
          <div className="flex gap-1.5">
            {FILTERS.map((f) => (
              <Button
                key={f.key}
                size="sm"                variant={filter === f.key ?"default":"outline"}
                className={cn(
              ...
              {filtered.length === 0 ? (
                ...
                filtered.map((athlete) => {
                  ...
                  return (
                    <TableRow
                      key={athlete.id}
                      className="cursor-pointer group"                      onClick={() => navigate(`/coach/athlete/${athlete.id}`)}
                    >
                      {/* Athlete */}
                      <TableCell>
