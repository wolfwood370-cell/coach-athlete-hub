...
function DraggableExercise({ exercise, onEdit, onArchive, isArchiving }: DraggableExerciseProps) {
  ...
  return (
    <div
      ...
      style={style}
      className={cn(
        "flex items-center p-2.5 rounded-lg bg-card border border-border/50 hover:border-primary/30 hover:bg-primary/5 transition-all duration-150 group mx-0 px-0 gap-0",
          ...
          <div className="flex flex-wrap gap-0.5 mt-0.5">
            {exercise.muscles.slice(0, 2).map((muscle) => (
              <Badge
                key={muscle}
                variant="secondary"
                className="text-4xs h-4 px-1 bg-primary/10 text-primary border-0"
              >
                {muscle.split(" ")[0]}
              </Badge>
            ))}
            {exercise.muscles.length > 2 && (
              <Badge variant="outline" className="text-4xs h-4 px-1">
                +{exercise.muscles.length - 2}
              </Badge>
            )}
          </div>
...
export function ExerciseLibrarySidebar({ className }: ExerciseLibrarySidebarProps) {
  ...
  const [formTrackingFields, setFormTrackingFields] = useState<string[]>([
    "sets",
    "reps",
    "weight",
  ]);
  ...
  const createMutation = useMutation({
    mutationFn: async (
      exercise: Omit<LibraryExercise, "id" | "coach_id" | "created_at" | "updated_at">,
    ) => {
  ...
  const filteredExercises = useMemo(() => {
    return exercises.filter((ex) => {
      ...
      const matchesMuscle =
        selectedMacroMuscle === "all" || ex.muscles.some((m) => selectedMacroMuscles.includes(m));
    ...
    selectedPattern,
    selectedMacroMuscle,
    selectedMacroMuscles,
  ]);

  // Check if any filter is active
  const hasActiveFilters =
    selectedType !== "all" || selectedPattern !== "all" || selectedMacroMuscle !== "all";
          ...
          {isLoading ? (
            ...
            <div className="text-center py-8">
              ...
              <p className="text-sm text-muted-foreground">
                {exercises.length === 0 ? "Nessun esercizio" : "Nessun risultato"}
              </p>
              {exercises.length === 0 && (
                <Button
                  variant="link"
            ...
            <>
              <div className="flex items-center justify-between px-1 mb-2">
                ...
                <span className="text-3xs text-muted-foreground">Trascina nella griglia →</span>
              </div>
