...
function generateHealthSummary({
  ...
  if (painTests.length > 0) {
    ...
    painTests.forEach(test => {
      details.push(`⚠️ FMS Dolore: ${test.testName} (${test.bodyArea})`);
    });
  }
  ...
  if (dysfunctionalTests.length > 0) {
    ...
    dysfunctionalTests.forEach(test => {
      const sideInfo = test.leftScore !== null && test.rightScore !== null
        ...
        : "";
      details.push(`⚠️ FMS Disfunzionale: ${test.testName}${sideInfo}`);
    });
  }
  ...
  if (limitedTests.length > 0 && status === "green") {
    ...
    limitedTests.forEach(test => {
      details.push(`ℹ️ FMS Limitato: ${test.testName} (Score: ${test.minScore}/3)`);
    });
  }
  ...
  switch (status) {
    ...
    case "green":
      ...
      if (!fmsTestDate) {
        ...
        details.push("ℹ️ Test FMS non ancora eseguito");
      } else {
